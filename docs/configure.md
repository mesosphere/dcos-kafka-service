...
1.  In the dialog that appears, click the **Environment** tab and update any field(s) to their desired value(s). For example, to [increase the number of Brokers][8], edit the value for `BROKER_COUNT`. Do not edit the values for `FRAMEWORK_NAME`, `BROKER_DISK`, `PLACEMENT_STRATEGY`, or `ENABLE_BROKER_HEALTH_CHECK`.
