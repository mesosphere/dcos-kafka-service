...
EPILOGUE_STRING = (
    "In addition, environment variables named 'TEMPLATE_SOME_PARAMETER' "
...
def main(argv):
    parser = argparse.ArgumentParser(description=DESCRIPTION_STRING, epilog=EPILOGUE_STRING)
    ...
    parser.add_argument(
        "package_dir_path", type=str, help="The local path where the package template is located"
    )
    parser.add_argument(
        "upload_dir_url",
        type=str,
    ...
    parser.add_argument(
        "artifact_paths", type=str, nargs="+", help="The artifact paths (for sha256 as needed)"
    )
