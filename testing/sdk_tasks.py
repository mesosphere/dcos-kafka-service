...
def get_summary(with_completed=False):
    ...
    task_lines = sdk_cmd.run_cli(
        "task --all" if with_completed else "task", print_output=False
    ).split("\n")
    output = []
...
def get_tasks_avoiding_scheduler(service_name, task_name_pattern):
    ...
    server_tasks = [task for task in get_summary() if task_name_pattern.match(task.name)]
...
def check_task_relaunched(task_name, old_task_id, timeout_seconds=DEFAULT_TIMEOUT_SECONDS):
    ...
    def fn():
        try:
            task_ids = set(
                [t["id"] for t in shakedown.get_tasks(completed=True) if t["name"] == task_name]
            )
        except dcos.errors.DCOSHTTPException:
...
def check_task_not_relaunched(
    service_name, task_name, old_task_id, timeout_seconds=DEFAULT_TIMEOUT_SECONDS
):
