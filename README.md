...
``` bash
$ curl -X PUT "$DCOS_URI/service/kafka0/v1/topics/topic1?operation=config&key=foo&value=bar"
...
``` bash
$ curl -X PUT "$DCOS_URI/service/kafka0/v1/topics/topic1?operation=deleteConfig&key=foo"
```
