...
def list_plans(service_name, timeout_seconds=TIMEOUT_SECONDS):
    return sdk_cmd.service_request(
        "GET", service_name, "/v1/plans", timeout_seconds=timeout_seconds
    ).json()
...
def get_plan(service_name, plan, timeout_seconds=TIMEOUT_SECONDS):
    ...
    def wait_for_plan():
        response = sdk_cmd.service_request(
            "GET", service_name, "/v1/plans/{}".format(plan), retry=False, raise_on_error=False
        )
...
def wait_for_kicked_off_plan(service_name, plan_name, timeout_seconds=TIMEOUT_SECONDS):
    return wait_for_plan_status(
        service_name, plan_name, ["STARTING", "IN_PROGRESS"], timeout_seconds
    )
